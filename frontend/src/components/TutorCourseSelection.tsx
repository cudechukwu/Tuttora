...
import { Search, BookOpen, ChevronDown, ChevronUp, Check, Building, Users, Trash2 } from 'lucide-react';
...
interface TutorCourseSelectionProps {
...
export default function TutorCourseSelection({ 
  selectedCourses, 
  onCourseSelectionChange,
  selectedDepartments,
  onDepartmentSelectionChange,
  maxCourses = 10 
}: TutorCourseSelectionProps) {
  ...
  const handleDepartmentToggle = (department: string) => {
    if (selectedDepartments.includes(department)) {
      onDepartmentSelectionChange(selectedDepartments.filter(d => d !== department));
    } else {
      onDepartmentSelectionChange([...selectedDepartments, department]);
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {selectedCourses.length > 0 && (
          ...
          <h3 className="text-xs font-semibold text-pink-800 mb-3">
            Selected Courses ({selectedCourses.length}/{maxCourses})
          </h3>
          <div className="space-y-2">
            {selectedCourses.map((course) => (
              <div key={course.id} className="flex items-center justify-between bg-white rounded-lg p-1.5 border border-pink-200">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          ...
          <div className="space-y-2">
            {selectedDepartments.map((department) => (
              <div key={department} className="flex items-center justify-between bg-white rounded-lg p-1.5 border border-blue-200">
                <div className="flex-1">
                  <div className="flex items-center space-x-2">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4">
        ...
        {showDepartmentSearch ? (
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between bg-primary-50 border border-primary-200 rounded-lg p-3">
              ...
              <div className="flex items-center space-x-2">
                <button
                  onClick={() => handleDepartmentToggle(selectedDepartment)}
                  className={`px-3 py-1 rounded-lg text-xs font-medium transition-colors ${
                    selectedDepartments.includes(selectedDepartment)
                      ? 'bg-blue-600 text-white'
                      : 'bg-white text-blue-600 border border-blue-300 hover:bg-blue-50'
                  }`}
                >
                  {selectedDepartments.includes(selectedDepartment) ? 'Selected' : 'Select All'}
                </button>
                <button
            ...
            <div ref={courseSearchRef} className="relative">
              <label className="block text-xs font-medium text-gray-700 mb-2">
                Or select specific courses in {selectedDepartment}
              </label>
              <div className="relative">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400" />
                <input
                        ...
                        return (
                          <div
                            ...
                            className={`px-3 py-2 cursor-pointer ${
                              isSelected
                                ? 'bg-pink-50 text-pink-900' // changed from green to pink
                                : isDisabled
                          ...
                          >
                            <div className="flex items-center justify-between">
                              <div className="flex-1">
                                <div className="flex items-center space-x-2">
                                  {isSelected && (
                                    <Check className="w-4 h-4 text-pink-600 flex-shrink-0" /> // changed from green to pink
                                  )}
        ...
        <div className="text-xs text-gray-600 bg-blue-50 border border-blue-200 rounded-lg p-3 mt-4">
          ...
          <ul className="space-y-1 text-blue-700">
            <li>• Select specific courses you feel confident tutoring</li>
            <li>• Or choose all courses in this department</li>
            <li>• Tutoring requests will match your course/department selections</li>
          </ul>
        </div>
      </div>
    </div>
